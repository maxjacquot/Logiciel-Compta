...
class ComptaAuto:
    ...
    def fillJour(self, lignecourrante, worksheet, file):
        ...
        worksheet["F" + lignecourrante].value = (
            valRL  ## remboursemenent loto car 2 cas et impossible de faire .value + .value
        )
...
class Application(tk.Tk):
    ...
    def AskComptaFile(self):
        ...
        FileC = filedialog.askopenfilename(
            initialdir="Desktop/", title="Feuille de compta"
        )
        print(FileC)
    ...
    def AskPackFile(self):
        FileP = filedialog.askdirectory(
            initialdir="Desktop/", title="Paquet de feuille de caisse"
        )
        if FileP != "":
            self.canva1.create_oval(
                360, 105, 390, 135, fill="green"
            )  ## voyant check Paquet de copie
            return FileP
        else:
            self.canva1.create_oval(
    ...
    def AskMA(self):
        ...
        self.canva2 = tk.Canvas(self, width=200, height=75)
        self.canva2.create_text(
            100, 25, text=str(mois) + " " + str(annee), font=tkinter.font.Font(size=12)
        )  ## Titre dans le rectangle
        self.canva2.place(x=200, y=225)

    def affichage_message(
        self, message, titre="Compta informations", largeur=500, hauteur=100
    ):
        # Création d'une nouvelle fenêtre Toplevel avec self comme parent
        ...
        # Conteneur principal pour un placement plus souple
        cadre = tk.Frame(fenetre, padx=20, pady=20)
        cadre.pack(expand=True, fill="both")

        # Affichage du message
        etiquette = tk.Label(cadre, text=message)
        etiquette.pack(pady=(0, 10))

        # Bouton OK qui ferme la fenêtre
        bouton_ok = tk.Button(cadre, text="OK", width=10, command=fenetre.destroy)
        bouton_ok.pack()

        # Liaison de la touche Entrée pour fermer la fenêtre
        fenetre.bind("<Return>", lambda event: fenetre.destroy())

        # Attendre que la fenêtre soit fermée avant de continuer
        self.wait_window(fenetre)

    def TestCompta(self):
        if self.Compta.tablfile == "":
            self.affichage_message("Vous n'avez pas donné de tableur de compta")

        elif self.Compta.paquetfile == "":
            self.affichage_message(
                "Vous n'avez pas donné de Paquet de feuille de caisse"
            )

        elif self.Compta.mois == "":
            self.affichage_message("Vous n'avez pas donné le mois ")

        elif self.Compta.annee == "":
            self.affichage_message("Vous n'avez pas donné l'année ")

        elif (
            self.Compta.tablfile != ""
            and self.Compta.paquetfile != ""
            and self.Compta.mois != ""
            and self.Compta.annee != 0
        ):
            self.Compta.Execution()
            self.affichage_message("Compta effectuée")
            print(" COMPTA FINI")
